<script lang="ts">
	...
	const weapon = itemName.split(" | ")[0].replace("★ ", "").replace("StatTrak™ ", "").replace("Souvenir ", "").trim().replace(" ", "_");
	const patternPromise: Promise<BetterBuff.CHPatterns> = fetch(browser.runtime.getURL("/ch_patterns.json")).then((res) => res.json());
</script>
