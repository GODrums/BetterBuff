...
export async function adjustGoodsSellOrder(apiData: BuffTypes.SellOrder.Data | undefined) {
	...
	for (let i = 0; i < rows.length; i++) {
		...
		if (listingDifferenceStyle > 0) {
			...
			const priceContainer = row.querySelector('p.hide-cny')?.parentElement;
			if (!priceContainer) return;
			priceContainer.insertAdjacentHTML(
				'beforeend',
				getListingDifference(Number.parseFloat(item.price), Number.parseFloat(goods_info.steam_price_cny), listingDifferenceStyle, steamTax, profitThreshold, listingDenominator, true)
			);
		}
...
async function adjustListingOptions(
	...
	if (listingOptions.copyGen) {
		const aCopyGen = genCopyGenButton(
			weaponSchema,
			item.asset_info.info.paintindex,
			item.asset_info.info.paintseed,
			item.asset_info.paintwear,
			item.asset_info?.info?.stickers ?? [],
			goods_info.market_hash_name.includes('StatTrak™')
		);
		elementsToAdd.push(aCopyGen);
	}

	if (listingOptions.matchFloat) {
		const min = Number.parseFloat(item.asset_info.paintwear.slice(0, 5));
		const max = min + 0.001;
