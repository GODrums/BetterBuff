...
export async function adjustItemDetails(apiData: BuffTypes.ItemDescDetail.Data) {
	...
	if (weaponSchema) {
		...
		const min = Number.parseFloat(assetInfo.paintwear.slice(0, 5));
		const max = min + 0.001;
...
export async function adjustSearchPage(apiData: BuffTypes.MarketGoods.Data) {
	...
	for (let i = 0, l = cards.length; i < l; i++) {
		...
		if (priceContainer) {
			const sellingPriceCNY = Number.parseFloat(item.sell_min_price);
			const buyingPriceCNY = Number.parseFloat(item.buy_max_price);
			...
			if (listingDifferenceStyle > 0) {
				...
				const platformPrice = Number.parseFloat(listingDenominator === 1 ? item.buy_max_price : item.goods_info.steam_price_cny);
...
export function adjustTopBookmarked(apiData: BuffTypes.TopPopular.Data) {
	...
	for (let i = 0, l = cards.length; i < l; i++) {
		...
		if (weaponSchema) {
			...
			const aCopyGen = document.createElement('a');
