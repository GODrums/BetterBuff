...
export namespace ExtensionStorage {
	export const enabled = storage.defineItem<IStorage['enabled']>('local:enabled', {
		defaultValue: defaultStorage.enabled,
	});

	export const darkMode = storage.defineItem<IStorage['darkMode']>('local:darkMode', {
		defaultValue: defaultStorage.darkMode,
	});

	export const hideFloatBar = storage.defineItem<IStorage['hideFloatBar']>('local:hideFloatBar', {
		defaultValue: defaultStorage.hideFloatBar,
	});

	export const layoutFix = storage.defineItem<IStorage['layoutFix']>('local:layoutFix', {
		defaultValue: defaultStorage.layoutFix,
	});

	export const showBigPreviews = storage.defineItem<IStorage['showBigPreviews']>('local:showBigPreviews', {
		defaultValue: defaultStorage.showBigPreviews,
	});

	export const dataProtection = storage.defineItem<IStorage['dataProtection']>('local:dataProtection', {
		defaultValue: defaultStorage.dataProtection,
	});

	export const preferredPayments = storage.defineItem<IStorage['preferredPayments']>('local:preferredPayments', {
		defaultValue: defaultStorage.preferredPayments,
	});

	export const listingDenominator = storage.defineItem<IStorage['listingDenominator']>('local:listingDenominator', {
		defaultValue: defaultStorage.listingDenominator,
	});

	export const listingDifferenceStyle = storage.defineItem<IStorage['listingDifferenceStyle']>('local:listingDifferenceStyle', {
		defaultValue: defaultStorage.listingDifferenceStyle,
	});

	export const platformTax = storage.defineItem<IStorage['platformTax']>('local:steamTax', {
		defaultValue: defaultStorage.platformTax,
	});

	export const profitThreshold = storage.defineItem<IStorage['profitThreshold']>('local:profitThreshold', {
