...
export function genListingAge(created_at: number) {
	const date = new Date(created_at * 1_000);
	const timeEpoch = Date.now() - created_at * 1_000;
	const dateHours = Math.floor(timeEpoch / 3_600_000);
	...
	const dateText = `${dateHours < 49 ? `${dateHours} hour${dateHours === 1 ? "" : "s"}` : `${Math.floor(timeEpoch / 3_600_000 / 24)} days`} ago`;
